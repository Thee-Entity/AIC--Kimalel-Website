...
const heroSlides = [
  PlaceHolderImages.find(p => p.id === 'ministry-youth'),
  PlaceHolderImages.find(p => p.id === 'event-2'),
].filter(Boolean) as (typeof PlaceHolderImages)[0][];

export default function YouthFellowshipHero() {
    ...
    <section className="relative h-[50vh] flex items-center justify-center text-white text-center overflow-hidden">
      ...
      <div className="relative z-10 px-4 flex flex-col items-center">
        ...
        <p className="text-lg md:text-xl max-w-3xl mx-auto">
            A vibrant community for young believers to connect, grow, and serve.
        </p>
      </div>
    </section>
  );
